...
export default function Scoreboard() {
  ...
  const saveScoreboardData = async (data) => {
    try {
      const scoreboardDataJSON = JSON.stringify(data);
      await AsyncStorage.setItem(SCOREBOARD_STORAGE_KEY, scoreboardDataJSON);
    } catch (error) {
      console.error('Error saving scoreboard data:', error);
    }
  };

  // Example: Add a new scoreboard entry
  const addScoreboardEntry = async () => {
    const newEntry = {
      playerName: 'John Doe',
      totalPoints: 100,
      dateTime: new Date().toString(),
    ...
    // Retrieve the existing scoreboard data
    let updatedScoreboardData = [...scoreboardData];
    updatedScoreboardData.push(newEntry);

    // Sort the data based on totalPoints (you can customize the sorting logic)
    updatedScoreboardData.sort((a, b) => b.totalPoints - a.totalPoints);

    // Keep only the top NBR_OF_SCOREBOARD_ROWS entries
    if (updatedScoreboardData.length > NBR_OF_SCOREBOARD_ROWS) {
      updatedScoreboardData = updatedScoreboardData.slice(0, NBR_OF_SCOREBOARD_ROWS);
    }

    // Update the state with the new data
    setScoreboardData(updatedScoreboardData);

    // Save the updated scoreboard data to AsyncStorage
    saveScoreboardData(updatedScoreboardData);
  };
        ...
        <View style={styles.container}>
          ...
          {scoreboardData.map((entry, index) => (
            <View style={styles.entry} key={index}>
              ...
              <Text style={styles.totalPoints}>{entry.totalPoints} points</Text>
              <Text style={styles.dateTime}>{entry.dateTime}</Text>
            </View>
          ))}
        </View>
        <Button title="Add Entry" onPress={addScoreboardEntry} />
        <Footer />
      </>
    </BackgroundContainer>
  );
